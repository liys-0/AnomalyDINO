...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        default=[1],  # action=IntListAction,
...
if __name__ == "__main__":
    ...
    for shot in list(args.shots):
        ...
        if args.faiss_on_cpu:
            print(
                "Warning: Running similarity search on CPU. Consider using faiss-gpu for faster inference."
            )
        ...
        for seed in seeds:
            ...
            if os.path.exists(f"{results_dir}/metrics_seed={seed}.json"):
                ...
                with open(measurements_file, "w", newline="") as file:
                    ...
                    for object_name in objects:
                        if save_examples:
                            ...
                            os.makedirs(
                                f"{plots_dir}/{object_name}/examples", exist_ok=True
                            )

                        # CUDA warmup
                        for _ in trange(
                            args.warmup_iters, desc="CUDA warmup", leave=False
                        ):
                            first_image = os.listdir(
                                f"{args.data_root}/{object_name}/train/good"
                            )[0]
                ...
                print(
                    f"Finished AD for {len(objects)} objects (seed {seed}), mean inference time: {sum(inference_times) / len(inference_times):.5f} s/sample = {len(inference_times) / (sum(inference_times) + 1e-10):.2f} samples/s"
                )
                ...
                for object_name in objects:
                    ...
                    if not os.path.exists(good_folder):
                        print(
                            f"Warning: 'good' folder not found for {object_name}! No evaluation will be performed for seed {seed}."
                        )
                        print(
                            "Finished AD without evaluation, inference results saved to",
                            results_dir,
                        )
                        break
                else:
